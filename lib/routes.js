...
exports.getTurnts = function(req, res){
  var offset = req.query.offset || 0;

  db.getMany({ offset : offset }).then(function(turnts){
    res.render('turnts', {
      offset  : offset,
      rootUrl : config.ROOT_URL,
      turnts  : turnts,
...
exports.createTurnt = function(req, res){
  ...
  if(req.query.key == config.TURNT_KEY){
    db.remember({
      ...
      console.log('sending 200 and attempting to text');
      res.sendStatus(200);
      texter.sendURL({
        phone : req.query.phone,
        url   : config.ROOT_URL + '/t/' + req.query.s3Id
      });
    }).catch(function(e) {
      console.log('error:', e);
    });
